...
class RansPgnTest extends Specification:
  ...
  "game compression" should {
    "compress and decompress" in {
      forall(fixtures) { pgn =>
        ...
        val encoded  = encoder.encodeWithRans(pgnMoves)
        val decoded  = encoder.decodeWithRans(encoded, pgnMoves.size)
    ...
    "unmoved rooks" in {
      ...
      val d1 = encoder.decodeWithRans(encoded, 0)
      d1.unmovedRooks.asScala must_== Set(0, 7, 56, 63)

      val d2 = encoder.decodeWithRans(encoded, pgnMoves.size)
    ...
    "half-move clock" in {
      ...
      (0 to pgnMoves.size).map(encoder.decodeWithRans(encoded, _).halfMoveClock) must_== halfMoveClocks
    }
    "last uci" in {
      val pgnMoves =
      ...
      val empty = encoder.decodeWithRans(encoded, 0)
      Option(empty.lastUci) must beNone

      val decoded = encoder.decodeWithRans(encoded, pgnMoves.size)
      Option(decoded.lastUci) must beSome("f6e6")
    }
    ...
    "position hash 1. a4 b5 2. h4 b4 3. c4 bxc3 4. Ra3" in {
      ...
      val d7 = encoder.decodeWithRans(encoded, 7)
