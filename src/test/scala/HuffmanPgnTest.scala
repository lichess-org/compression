...
class HuffmanPgnTest extends Specification:
  lazy val encoder: Encoder = Encoder()
  ...
  "game compression" should {
    "compress and decompress" in {
      forall(fixtures) { pgn =>
        ...
        val encoded = encoder.encodeWithHuffman(pgnMoves)
        val decoded = encoder.decodeWithHuffman(encoded, pgnMoves.size)
    ...
    "stable format" in {
      forall(v1 zip fixtures) { case (encoded, pgn) =>
        val pgnMoves = pgn.split(" ")
        val decoded = encoder.decodeWithHuffman(base64ToBytes(encoded), pgnMoves.size)
        pgnMoves must_== decoded.pgnMoves
      }
    }
    ...
    "least surprise" in {
      val n = 22
      val decoded = encoder.decodeWithHuffman(Array.fill(n)(0.toByte), n)
    ...
    "unmoved rooks" in {
      ...
      val d1 = encoder.decodeWithHuffman(encoded, 0)
      d1.unmovedRooks.asScala must_== Set(0, 7, 56, 63)

      val d2 = encoder.decodeWithHuffman(encoded, pgnMoves.size)
    ...
    "half-move clock" in {
      ...
      (0 to pgnMoves.size).map(encoder.decodeWithHuffman(encoded, _).halfMoveClock) must_== halfMoveClocks
    }
    ...
    "last uci" in {
      ...
      val empty = encoder.decodeWithHuffman(encoded, 0)
    ...
    "position hash 1. a4 b5 2. h4 b4 3. c4 bxc3 4. Ra3" in {
      ...
      val d7 = encoder.decodeWithHuffman(encoded, 7)
